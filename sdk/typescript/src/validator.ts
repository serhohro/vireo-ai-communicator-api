...
export class Validator {
  ...
  constructor() {
    ...
    this.rules.push(
      new MessageTypeRule([
        // v3.1 uppercase intents
        'PROPOSE', 'COMMIT', 'EXECUTE', 'VERIFY', 'ESCALATE', 'DONE',
        'FAILED', 'TIMEOUT', 'DISCOVER', 'NEGOTIATE', 'REJECT',
        'CANCELLED', 'ACK', 'NACK', 'QUERY', 'RESPONSE', 'ERROR',
        // v3.0 lowercase (backward compat)
        'propose', 'commit', 'execute', 'verify', 'escalate', 'done',
        'failed', 'ack', 'nack', 'query', 'response', 'error',
      ]),
      new VersionRule('3.1.0', '3.1.9'),
      // TTLRule removed — TTL not in wire format v3.1
    );
  }
